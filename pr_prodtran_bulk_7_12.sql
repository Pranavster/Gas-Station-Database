...
-- 3rd: in the loop, update begining_pos = lf_char_pos + 1 and find the new position of the end position based on the substring starting from begining_pos to the end of the line.
    ...
    #v_pid	date	quantity	price	name
    ...
    read_lines: loop
        ...
        if exists( select * from sales where prod_id = v_pid) then
			select "before update";
            update sales set prod_date = v_date, quantity = v_quan, prod_value = v_price, addby = v_user where prod_id = v_pid;
            select "after update";
		else
			insert into sales(prod_id, prod_date, quantity, prod_value, addby, addwhen, modby, modwhen, rate) values(v_pid,v_date,v_quan,v_price,v_user,sysdate(),"Pranav",NULL,5.33);
		end if;
        -- end loop read_line;
        set ctr = ctr + 1;
        if(ctr > 9) then
