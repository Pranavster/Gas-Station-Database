CREATE DEFINER=`root`@`127.0.0.1` PROCEDURE `pr_monthly_report`(from_year_month varchar(7), to_year_month varchar(7))
begin
	...
	select s.*, p.prod_name, p.prod_type from sales s join product p on s.prod_id = p.prod_id 
    where (p.addwhen >= from_year_month and p.addwhen < v_to_year_month);
end