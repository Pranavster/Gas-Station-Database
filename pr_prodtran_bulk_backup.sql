...
-- 3rd: in the loop, update begining_pos = lf_char_pos + 1 and find the new position of the end position based on the substring starting from begining_pos to the end of the line.
    ...
    read_lines: loop
        ...
        select 62 ln_62, this_delim, next_delim, substring(v_line,this_delim, next_delim), REGEXP_INSTR( substring(v_line,this_delim, next_delim), "[^0-9]" ); 
        if ((REGEXP_INSTR( substring(v_line,this_delim, next_delim - 1), "[^0-9-+]")) = 0 ) then #tab out of 0-9 so we dont want to include it
